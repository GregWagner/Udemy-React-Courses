...
function HomeRoute() {
    ...
    const {data: cPlusPlusRepos} = useRepositories('stars:>10000 language:c++');
    const {data: cSharpRepos} = useRepositories('stars:>10000 language:c#');
    ...
    return (
        <div>
            ...
            <div className="container mx-auto py-8 grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <RepositoriesTable label="Most Popular C++" repositories={cPlusPlusRepos}/>
                <RepositoriesTable label="Most Popular C#" repositories={cSharpRepos}/>
            </div>
        </div>
    );
}
